...
class KoaPug {
    ...
    set locals(value) {
        assert_1.default(is_plain_obj_1.default(value), `"locals" requires a plain object`);
    ...
    set options(options) {
        if (!options || !is_plain_obj_1.default(options)) {
            return;
        }
        ...
        if (is_plain_obj_1.default(options.locals)) {
            this.defaultLocals = options.locals;
        }
        // tslint:disable-next-line
        if (typeof options.helperPath === 'string' || Array.isArray(options.helperPath)) {
            this.helpers = extend_1.default({}, this.helpers, load_helpers_1.default(options.helperPath));
        }
        const nonPugKeys = ['viewPath', 'locals', 'helperPath', 'app'];
        for_in_1.default(options, (value, key) => {
    ...
    async render(tpl, locals, options) {
        const compileOptions = extend_1.default({}, this.defaultOptions);
        if (is_plain_obj_1.default(options)) {
            extend_1.default(compileOptions, options);
        }
    ...
    use(app) {
        ...
        app.context.render = async function (tpl, locals, options) {
            const ctx = this;
            const finalLocals = extend_1.default({}, self.helpers, self.defaultLocals, ctx.state, locals);
